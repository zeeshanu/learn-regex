...
<<<<<<< HEAD
捕获组是一组写在 `(...)` 中的子模式。例如之前说的 `{}` 是用来表示前面一个字符出现指定次数。但如果在 `{}` 前加入捕获组则表示整个组内的字符重复 N 次。例如，表达式 `(ab)*` 匹配连续出现 0 或更多个 `ab`。
=======
...
值得注意的是，捕获组不只是进行匹配，同时捕获的分组还可应用于正则所运行的语言中。例如 Python、Javascript 或其他任何支持正则的语言。

### 2.5.1 非捕获组

非捕获组也是一种捕获组，不同之处是非捕获组只进行匹配而不分组。非捕获组用写在 `(...)` 中的 `?:` 来表示。例如：`(?:c|g|p)ar` 与 `(c|g|p)ar` 匹配相同的字符，但不会创建分组。
...
非捕获组通会在查找替换功能或与捕获组配合来描述分组特征中起到作用。 参考 [4. 零宽度断言（前后预查）](#4-零宽度断言前后预查).
